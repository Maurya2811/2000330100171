...
const TrainDataComponent = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-black">
      ...
      <ul className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-1">
        {sortedData.map((train, index) => (
          <li
            key={index}
            className="bg-purple-100 dark:bg-gray-700 shadow-md rounded-lg p-4 text-gray-800 dark:white"
          >
            <h3 className="text-xl font-semibold mb-2 text-white-600 dark:text-white">
              {train.trainName}
            </h3>
            ...
            <p className="text-sm text-yellow-600 dark:text-white">
              Sleeper Seats Available: {train.seatsAvailable.sleeper}
            </p>
            <p className="text-sm text-red-600 dark:text-white">
              AC Seats Available: {train.seatsAvailable.AC}
            </p>
            <p className="text-sm text-indigo-600 dark:text-white">
